...
Status: **IN_PROGRESS / EXECUTION_TRANSPORT_RESTORED / DIAGNOSTIC_COMPATIBILITY_DEBT** — explicitly resumed by user on 2026-09-16 because the shared gateway blocked Deep Research semantic verification. The live gateway had drifted from the CURRENT AgentField DEV control plane: PID1 hard-coded obsolete upstream hostname `control-plane-edshqtkwskg3lrczekhcmd71-084728200638` while CURRENT control plane is `control-plane-edshqtkwskg3lrczekhcmd71-115125416151`. The bounded Coolify service config was updated in place to that CURRENT upstream and only the gateway service was recreated/restarted. Fresh discovery now PASSes with 4 agents / 73 reasoners, including active `meta_deep_research`; async reasoner execution and terminal status readback PASS. The current repair restores execution but still binds to an ephemeral generated container name, so durable alias/target resolution remains debt rather than a claimed permanent fix. Stale `/api/ui/v1/...` diagnostic routes also remain a separate contract-compatibility debt.
