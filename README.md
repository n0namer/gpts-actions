...
`0.2.2` is a provisional publication-schema normalization release. It adds explicit `x-openai-isConsequential` metadata to all 14 operations and removes several empty schema objects, but the known-working reference contracts use both annotated and unannotated operations, so those changes are not evidence by themselves of GPT Builder compatibility. Node process logs intentionally retain the native bounded `application/x-ndjson` wire format; this is the only response media type that differs from the selected JSON-only working reference contracts and must be confirmed by Builder import/Preview before final acceptance.
